...
export default function ObjectEdit(props: Props) {
  ...
  const addText = async () => {
    ...
    // 检测图层是否存在
    const layer = await client.layers.getLayer(textLayerRef.current.layerName)
    if (!layer) return;
  ...
  const addPoint = async () => {
    ...
    // 检测图层是否存在
    const layer = await client.layers.getLayer(pointLayerRef.current.layerName)
    if (!layer) return;
  ...
  const addLine = async () => {
    ...
    // 检测图层是否存在
    const layer = await client.layers.getLayer(lineLayerRef.current.layerName)
    if (!layer) return;
  ...
  const addRegion = async () => {
    ...
    // 检测图层是否存在
    const layer = await client.layers.getLayer(regionLayerRef.current.layerName)
    if (!layer) return;
